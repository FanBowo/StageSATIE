...
bool Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, uint8_t value) {
    return LinuxWriteBytes(_address,(uint8_t)reg,1,&value);
...
uint8_t Adafruit_BNO055::read8(adafruit_bno055_reg_t reg) {
    uint8_t ValRead=0;
    int8_t ReadCount=0;
    ReadCount=LinuxReadBytes(_address,(uint8_t)reg,1,&ValRead);
    if (ReadCount>=0){
        return ValRead;
    }
    else{
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t *buffer,
                              uint8_t len) {
    int8_t ReadCount=0;
    ReadCount=LinuxReadBytes(_address,(uint8_t)reg,len,buffer);
//  _wire->beginTransmission(_address);
//#if ARDUINO >= 100
//  _wire->write((uint8_t)reg);
//#else
